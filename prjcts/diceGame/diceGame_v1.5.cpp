...
int main(int argv, char** argc)
{
	...
	do
	{
		if (snglRoll != true) {
			...
			{
				cout << "\x1B[5m" << " Winner! Triple Same Number!!" << "\x1B[25m" << endl; //three of same number 
				result = false;
				return 0;
			}
			...
			if (dice1 == dice2 && dice3 != dice1 && dice3 != 1) 
			{
				if (pnts > 0) {
					pnts2 = dice3;
					if (pnts > pnts2) {
			...
			else if (dice1 == dice3 && dice2 != dice1 && dice2 != 1) 
			{
				if (pnts > 0) {
					pnts2 = dice2;
					if (pnts > pnts2) {
			...
			else if (dice2 == dice3 && dice1 != dice2 && dice1 != 1) 
			{
				if (pnts > 0) {
					pnts2 = dice1;
					if (pnts > pnts2) {
		...
		if (result == true)
		{
			...
			if (init == 'Y') {
				snglRoll = false; // allow next roll
			}
			else {
				result = false; // end game
			}
		}
	} while (result == true);
	return 0;
