...
int main(int argv, char** argc)
{
	...
	int dice1,                              //dice 1
		 dice2,                              //dice 2
		 dice3,                              //dice 3
		 rndSeed = pow(2, 3) - 1,//2^3-1 = 7
		 rCount = 0,                    //roll counter
		 combo = 0;                    //combo counter
	float pnts = 0,                   //points for player 1
		pnts2 = 0;                      //points for other player 2
	char init;
	...
	do{
		if (snglRoll != true) {
			...
			{
				cout << "\x1B[5m" << "YOU LOST! Sequential 1-2-3!!" << "\x1B[25m" << endl;
				combo++;
				result = false;
			}
			...
			for (int i = 0; i < 1 && combo == 0; i++) {
				cout << "\x1B[5m" << "Indeterminate! (ReRoll)" << "\x1B[25m" << endl;
				snglRoll = false; //re-roll
			}
			// single roll process
			snglRoll = true;
			if (result == true)
			{
				cout << "\x1B[5m" << "Roll again? (Y/N): " << "\x1B[25m" << endl;
				cin >> init;
